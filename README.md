...
Published on PyPI via Trusted Publishing (`.github/workflows/publish.yml`, already
configured — no API token to manage). To ship a new version:
...
Creating the GitHub Release triggers the workflow automatically; PyPI accepts the
upload because it trusts this exact repo + workflow, no secret involved.
